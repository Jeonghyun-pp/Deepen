...
export default function ExportModal({ data, fullData, selectedNodeId, open, onClose }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
      <div className="bg-[color:var(--v2-ink-soft)]/95 backdrop-blur-md border border-white/10 rounded-2xl shadow-[0_20px_60px_-20px_rgba(0,0,0,0.8)] w-[560px] max-h-[80vh] flex flex-col">
        {/* Header */}
        <div className="flex items-center justify-between px-6 py-4 border-b border-white/10">
          <h2 className="text-sm font-bold text-white">지식 스냅샷 내보내기</h2>
        ...
        <div className="px-6 py-4 border-b border-white/10">
          <p className="text-xs font-semibold text-white/75 mb-2">범위</p>
          <div className="flex flex-col gap-1.5">
            {[
              ...
              <label key={key} className={`flex items-center gap-2 text-xs text-white/75 ${disabled ? "opacity-40" : ""}`}>
                <input
                  type="radio"
                  name="scope"
                  checked={scope === key}
                  ...
                  disabled={disabled}
                  className="accent-[color:var(--v2-green-soft)]"
                />
                {label}
              </label>
            ))}
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4">
          <p className="text-xs font-semibold text-white/75 mb-2">미리보기</p>
          <pre className="text-[11px] text-white/75 bg-white/5 rounded-xl p-4 overflow-x-auto whitespace-pre-wrap leading-relaxed border border-white/10 max-h-[300px] overflow-y-auto">
            {markdown}
          </pre>
        </div>
        ...
        <div className="flex gap-3 px-6 py-4 border-t border-white/10">
          <button
            onClick={handleCopy}
            className="flex-1 flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl border border-white/10 text-sm font-semibold text-white/75 hover:border-[color:var(--v2-green-soft)]/40 hover:text-[color:var(--v2-green-soft)] transition-colors"
          >
          ...
          <button
            ...
            className="flex-1 flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-[color:var(--v2-green)] text-black text-sm font-bold hover:bg-[color:var(--v2-green-soft)] transition-colors"
          >
            <Download size={14} />
            MD 다운로드
          </button>
